...
write_rds(ids, glue('{wd}/tmp/ids.rds'))
...
write_rds(df_ex, "{wd}/tmp/df_ex.rds" %>% glue)
write_csv(df_ex, "{wd}/tmp/df_ex.csv" %>% glue) # export for Ian
...
write_rds(df_dec_age, "{wd}/tmp/df_dec_age.rds" %>% glue)
...
df_dec_age_cause <- df_dec$decomposition_results_by_age_cause %>%
    ...
    mutate(
        age3 = x %>% cut(c(0,Inf), right = FALSE) %>%
            lvls_revalue(c("Total")),
        period = year.final %>% cut(c(2015, 2019, 2020)) %>%
            lvls_revalue(c("2015-19", "2019-20")),
...
write_rds(df_dec_age_cause, "{wd}/tmp/df_dec_age_cause.rds" %>% glue)
